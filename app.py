...
# =============================================================================
# CONFIG — TIMEZONES (IMPORTANT)
# - L'admin/maître de jeu SAISIT l'heure France (Europe/Paris)
# - Le verrouillage (match commencé) se fait selon l'heure France
# - Les joueurs voient FR + MA
# =============================================================================
...
def format_dt_fr(dt: datetime) -> str:
    """Formate une date timezone-aware en style FR court."""
...
def parse_kickoff_paris(kickoff_paris_str: str) -> datetime:
    """Parse 'YYYY-MM-DD HH:MM' en datetime tz-aware Europe/Paris."""
...
def format_kickoff_dual(kickoff_paris_str: str) -> str:
    """Affiche l'heure France + Maroc à partir d'un kickoff stocké en heure Paris."""
    try:
...
def kickoff_has_started_paris(kickoff_paris_str: str) -> bool:
    """True si maintenant >= coup d'envoi, en se basant sur l'heure France."""
    try:
...
def is_editable_until_kickoff_paris(kickoff_paris_str: str) -> bool:
    """Pronostic modifiable tant que le match n'a pas commencé (heure France)."""
    try:
        return now_paris() < parse_kickoff_paris(kickoff_paris_str)
    except Exception:
        return False
...
def edited_after_kickoff(timestamp_utc_str: str, kickoff_paris_str: str) -> bool:
    """Détecte un edit après le coup d'envoi, en comparant timestamp (UTC) vs kickoff (Paris)."""
    try:
...
.block-container { max-width: 1180px; padding-top: 1.5rem !important; }
...
.tm-logo-rounded img { width: 100%; height: 100%; object-fit: cover; }
...
.tm-chip-dot { width:7px; height:7px; border-radius:999px; background:#22c55e; }

div[data-testid="stTabs"] > div[role="tablist"] {
    gap: 0.6rem; padding-bottom: 0.25rem; border-bottom: none !important;
}
...
matches = Table(
    ...
    Column("kickoff_paris", String, nullable=False),  # SAISI EN HEURE FRANCE "YYYY-MM-DD HH:MM"
...
def compute_export_tables() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # ---------- DETAIL (7 derniers jours en heure PARIS) ----------
    if df_preds.empty and df_manual.empty:
    ...
    # Ajout des points manuels au détail
    ...
    if not df_manual_all.empty:
        ...
        if not df_manual_all.empty:
            manual_detail = pd.DataFrame({
                ...
                "kickoff_paris": df_manual_all["created_at"].str.slice(0, 16),  # pas idéal mais cohérent export
    ...
    # Filtre 7 derniers jours (référence = Paris)
    ...
    show = show.sort_values("_ko_paris", ascending=False)

    show["⚠️"] = show.apply(
        lambda r: (
            "⚠️"
            if isinstance(r.get("timestamp_utc", None), str)
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; gap:1.3rem;">
            <div>
                <div class="tm-chip" style="padding:.45rem 1.1rem; border-radius:999px; display:inline-flex; align-items:center; gap:.5rem;">
                    ...
                    <span style="font-size:0.7rem;font-weight:700;letter-spacing:0.04em;">
                        {current_name}
                    </span>
                </div>
                ...
                <div style="margin-top:0.5rem; font-size:0.85rem; color:#9ca3af;">
                    Horaires affichés: France (Paris) + Maroc (Casablanca) — verrouillage selon heure France
                </div>
            </div>
...
with tab_pronos:
    if df_matches.empty:
        ...
        # kickoff en datetime tz-aware Paris
    ...
    with tab_maitre:
        if not can_manage_matches:
            ...
            with tab_ajout:
                ...
                st.caption("⏱️ Le maître de jeu saisit **l'heure France (Europe/Paris)**. Le verrouillage est basé sur cette heure.")
                ...
                with st.form("form_add_match"):
                    ...
                    with c3:
                        ...
                        st.markdown(
                            """
                            <style>
                            .time-sep { font-size: 24px; font-weight: 700; text-align: center; margin-top: 8px; }
                            </style>
                            """,
                            unsafe_allow_html=True,
                        )
                        ...
                        kickoff = kickoff_dt.strftime("%Y-%m-%d %H:%M")  # stocké comme heure Paris (string)
                            ...
                            for _, m in df_matches_gm.iterrows():
                                ...
                                with st.expander(exp_label):
                                    ...
                                    with c2:
                                        ph = st.number_input(
                                            f"{m['home']} (dom.)",
                                            min_value=0, max_value=20, step=1, value=ph0,
                                            key=f"gm_ph_{target_user_id}_{match_id}",
                                        )
                                    with c3:
                                        pa = st.number_input(
                                            f"{m['away']} (ext.)",
                                            min_value=0, max_value=20, step=1, value=pa0,
            ...
            with tab_points:
                ...
                if df_users_points.empty:
                    ...
                    reason_input = st.text_input("Saisir une nouvelle raison", placeholder="Ex : Bonus fair-play, Retard, Pari spécial...") \
                        if reason_choice == "(Nouvelle raison)" else reason_choice
                    ...
                    if df_manual_points_df.empty:
                        ...
                        # filtre 7 jours (Paris)
                        nowp = now_paris().astimezone(timezone.utc)
                        min_dt_utc = nowp - timedelta(days=7)
                        df_manual_points_df = df_manual_points_df[pd.notna(df_manual_points_df["_created"]) & (df_manual_points_df["_created"] >= min_dt_utc)]
